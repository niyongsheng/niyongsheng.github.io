...
// extending prototypes element methods with some custom ones
Element.addMethods({
	...
	*/
	...
	// ** {{{ setVendorPrefixStyle(element, property, value) }}} **
	//
	...
	setVendorPrefixStyle: function(element, property, value) {
		if (property.match(/^webkit/)) {
			property = property.replace(/^webkit/, '');
		} else if (property.match(/^moz/)) {
			property = property.replace(/^moz/, '');
		} else if (property.match(/^ms/)) {
			property = property.replace(/^ms/, '');
		} else if (property.match(/^o/)) {
			property = property.replace(/^o/, '');
	...
	// ** {{{ addVendorEventListener(element, type, listener, useCapture) }}} **
	//
	...
	addVendorEventListener: function(element, type, listener, useCapture) {
		if (typeof(addEventListener) == 'function') {
			if (type.match(/^webkit/)) {
				type = type.replace(/^webkit/, '');
			} else if (type.match(/^moz/)) {
				type = type.replace(/^moz/, '');
			} else if (type.match(/^ms/)) {
				type = type.replace(/^ms/, '');
			...
			element.addEventListener('moz'+type, listener, useCapture);
			element.addEventListener('ms'+type, listener, useCapture);
	...
	// ** {{{ removeVendorEventListener(element, type, listener, useCapture) }}} **
	//
	...
	removeVendorEventListener: function(element, type, listener, useCapture) {
		if (typeof(removeEventListener) == 'function') {
			...
			element.removeEventListener('moz'+type, listener, useCapture);
			element.removeEventListener('ms'+type, listener, useCapture);
...
addVendorEventListener = function(type, listener, useCapture) {
	Element.Methods.addVendorEventListener(window, type, listener, useCapture);
}
removeVendorEventListener = function(type, listener, useCapture) {
	Element.Methods.removeVendorEventListener(window, type, listener, useCapture);
}

// legacy Element2 for old extended element methods
var Element2 = {};
...
AC.Storage = {
	...
	storageType: function() {
		if (!this._storageType) {
			if (document.cookie) this._storageType = 'cookie';

			try {
				if (window.localStorage) this._storageType = 'localStorage';
	...
	getItem: function(key){
		var value;
		return ((this.storageType() == 'localStorage') && (value = localStorage.getItem(key))) ? value : this.readCookie(key);
	},
	removeItem: function(key){
		if (this.storageType() == 'localStorage') localStorage.removeItem(key);
